...
This is the result of two NPR Serendipity Day projects centered around breaking news, in which we try to find other ways to present discrete facts that emerge as a breaking news situation develops. Currently, there are point people in the newsroom who compile and distribute regular emails about what is known and not known about a given breaking news situation. Here, we try to apply that in other forms and look into opportunities to share some of that with the public.
...
```
cd breaking-news-facts
npm install less universal-jst -g --prefix node_modules
mkvirtualenv --no-site-packages breaking-news-facts
